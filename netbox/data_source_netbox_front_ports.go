...
func dataSourceNetboxFrontPorts() *schema.Resource {
	return &schema.Resource{
		Read:        dataSourceNetboxFrontPortRead,
		Description: `:meta:subcategory:DCIM:`,
		Schema: map[string]*schema.Schema{
			"device_id": {
				Type:        schema.TypeInt,
				Required:    true,
				Description: "The ID of the device to get front ports for.",
			},
			"filter": {
				Type:     schema.TypeSet,
				Optional: true,
...
func dataSourceNetboxFrontPortRead(d *schema.ResourceData, m interface{}) error {
	api := m.(*providerState)
	...
	for _, v := range filteredFrontPorts {
		...
		if v.RearPort != nil {
			rearPortData := map[string]interface{}{
				"id":   v.RearPort.ID,
				"name": *v.RearPort.Name,
			}
			mapping["rear_port"] = []map[string]interface{}{rearPortData}
		}
		mapping["rear_port_position"] = v.RearPortPosition
		if v.Tags != nil {
			var tags []int64
